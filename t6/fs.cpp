...
int create_virtual_disc(char file_name[])
{
    ...
    u_int32_t possible_datablocks_num = (superblock_.disc_size - sizeof(superblock) - MAX_FILES_NUM * sizeof(inode)) / sizeof(datablock);
    superblock_.datablocks_num = (superblock_.disc_size - sizeof(superblock) - MAX_FILES_NUM * sizeof(inode) - possible_datablocks_num * sizeof(int)) / sizeof(datablock);
    superblock_.free_datablocks_num = superblock_.datablocks_num;
    superblock_.inodes_num = MAX_FILES_NUM;
    superblock_.free_inodes_num = MAX_FILES_NUM;
    ...
    inode root_inode;
    root_inode.directory = true;
    root_inode.used = true;
    root_inode.id = 0;
    superblock_.free_inodes_num -= 1;

    fwrite(&root_inode, sizeof(root_inode), 1, file);

    for(int i = 0; i < MAX_FILES_NUM - 1; ++i)
    {
        inode node;
