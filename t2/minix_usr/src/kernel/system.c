...
FORWARD _PROTOTYPE( int get_proc_grp, (message *m_ptr) );
FORWARD _PROTOTYPE( int set_proc_grp, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_GETGRP:  r = get_proc_grp(&m); break;
      case SYS_SETGRP:  r = set_proc_grp(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
PRIVATE int get_proc_grp(m_ptr)
message *m_ptr;
{
  struct proc* p;
  p = proc_addr(m_ptr->m1_i1);
  return p->proc_grp;
}

PRIVATE int set_proc_grp(m_ptr)
message *m_ptr;
{
  struct proc* p;
  int target_grp = m_ptr->m1_i2;
  if(target_grp == 1 || target_grp == 2)
  {
    p = proc_addr(m_ptr->m1_i1);
    p->proc_grp = target_grp;
    return p->proc_grp;
  }
  return ENOENT;
}
