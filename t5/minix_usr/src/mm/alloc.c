 ...
 */
  ...
  if(!WORST_FIT_FLAG)
    ...
    register struct hole *biggestHole, *biggestHole_prev_ptr = NIL_HOLE;
    hp = biggestHole = hole_head;
    /* Find biggest hole */
    while( hp != NIL_HOLE && hp->h_base < swap_base ){
      if( hp->h_len > biggestHole->h_len ){
        biggestHole = hp;
        biggestHole_prev_ptr = prev_ptr;
      }
      prev_ptr = hp;
      hp = hp->h_next;
    }
    /* If enough space - use it */
    if( biggestHole->h_len >= clicks ){
      old_base = biggestHole->h_base;
      biggestHole->h_base += clicks;
      biggestHole->h_len  -= clicks;

      if ( biggestHole->h_len == 0 ) del_slot(biggestHole_prev_ptr, biggestHole);
...
PUBLIC int do_hole_map()
{
	...
	int size = mm_in.m1_i1;
	phys_clicks buf[NR_HOLES * 2 + 1];
	int i = 0;
	while(hp != NIL_HOLE && hp->h_base < swap_base && i < size-1)
	{
		buf[i] = hp->h_len;
		buf[i+1] = hp->h_base;
		i += 2;
	...
	buf[i] = 0;
	sys_copy(MM_PROC_NR, D, (phys_clicks)buf, who, D, (phys_clicks)mm_in.m1_p1, (phys_clicks)mm_in.m1_i1);
	return i/2;
}
